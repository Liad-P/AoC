...
part1 = do
    ...
    let parsedResults = foldl (\acc line -> 
...
doCalcPart2 inputOperators inputNums
    | length inputNums == 1 = head inputNums 
    | head inputOperators == '*' = doCalcPart2 (tail inputOperators) ([((head inputNums) * (head (reverse $ take 2 inputNums)))] ++ (drop 2 inputNums))
    | head inputOperators == '+' = doCalcPart2 (tail inputOperators) ([((head inputNums) + (head (reverse $ take 2 inputNums)))] ++ (drop 2 inputNums))
    | head inputOperators == '|' = doCalcPart2 (tail inputOperators) ([read ((show $ head inputNums) ++ (show $ head (reverse $ take 2 inputNums)))] ++ (drop 2 inputNums))
...
-- parallelFold :: (a -> b) -> (b -> b -> b) -> b -> [a] -> b
-- parallelFold mapFunc reduceFunc initVal xs =
--   foldr reduceFunc initVal (parMap rseq mapFunc xs)
